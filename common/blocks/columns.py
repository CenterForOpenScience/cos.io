...
from common.blocks import COLUMN_CHOICES
    ...
    def render_form(self, value, prefix='', errors=None):
        ...
        if errors:
            if len(errors) > 1:
                ...
                # blocks
                raise TypeError('StreamBlock.render_form unexpectedly '
        ...
        list_members_html = [
            self.render_list_member(child.block_type, child.value,
                                    "%s-%d" % (prefix, i), i,
...
class RowBlock(blocks.ListBlock):
    ...
    @property
    def media(self):
        return forms.Media(js=[static('wagtailadmin/js/blocks/sequence.js'),
    ...
    def render_basic(self, value, context=None):
        ...
        return format_html("<div class='row margin-bottom-30'>{0}</div>",
                           children)
    ...
    def render_form(self, value, prefix='', errors=None):
        if errors:
            if len(errors) > 1:
                ...
                raise TypeError('ListBlock.render_form unexpectedly received '
                                'multiple errors')
